...
clear
nginx -v
sudo usermod -g www-data gardiyan
clear
echo "Nginx kuruldu"
sleep 2
...
wget -qO- https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo tee /etc/apt/keyrings/postgresql.asc > /dev/null
...
sleep 2
apt-get update -y 
apt-get install -y gcc g++ curl libcairo2-dev libjpeg-turbo-progs libjpeg62-turbo-dev libpng-dev libtool-bin libossp-uuid-dev libavcodec-dev libavutil-dev libswscale-dev freerdp2-dev libpango1.0-dev libssh2-1-dev libvncserver-dev libtelnet-dev libssl-dev libvorbis-dev libwebp-dev libpulse-dev libavformat-dev ffmpeg cowsay make openjdk-17-jdk
...
apt-get install -y unzip zip inxi net-tools
...
sleep 2
# Guacamole yapılandırma dizinlerini oluştur
...
sudo tar -xzvf apache-tomcat-9.0.73.tar.gz
sudo mv apache-tomcat-9.0.73 tomcat9
sudo rm apache-tomcat-9.0.73.tar.gz

# Tomcat9'u çalıştıracak kullanıcıyı oluştur (eğer yoksa)
if ! id -u tomcat >/dev/null 2>&1; then
    sudo useradd -r -m -U -d /opt/tomcat9 -s /bin/false tomcat
fi
sudo chown -R tomcat:tomcat /opt/tomcat9

sudo cp /etc/guacamole/guacamole.war /opt/tomcat9/webapps/

sudo cp /home/gardiyan/Gardiyan_Setup/gardiyan_v2_s1/guacamole_setup_new/server.xml /opt/tomcat9/conf/

echo "GUACAMOLE_HOME=/etc/guacamole" | sudo tee -a /opt/tomcat9/bin/setenv.sh
